...
def _validate_required_string(
    config_module, attr_name, min_length=10, display_name=None
):
    ...
    if not isinstance(value, str) or len(value) < min_length:
        logger.warning(
            f"{display_name} in config.py appears to be invalid (too short or not a string)."  # noqa: E501
        )
...
def _validate_url(config_module, attr_name, display_name=None):
    ...
    if not isinstance(value, str) or not value.startswith("http"):
        logger.warning(
            f"{display_name} in config.py appears to be invalid (should be a valid HTTP/HTTPS URL)."  # noqa: E501
        )
...
def _validate_rate_limiting(config_module):
    ...
    try:
        ...
        if new_rate_limit_seconds <= 0:
            logger.warning(
                f"rate_limit_seconds in config ('{custom_rate_limit_seconds}') must be positive. Using default."  # noqa: E501
            )
            new_rate_limit_seconds = 10
    except (ValueError, TypeError):
        logger.warning(
            f"Invalid rate_limit_seconds in config ('{custom_rate_limit_seconds}'), using default."  # noqa: E501
        )
        new_rate_limit_seconds = 10
    ...
    try:
        ...
        if new_max_requests_per_minute <= 0:
            logger.warning(
                f"max_requests_per_minute in config ('{custom_max_requests_per_minute}') must be positive. Using default."  # noqa: E501
            )
            new_max_requests_per_minute = 6
    except (ValueError, TypeError):
        logger.warning(
            f"Invalid max_requests_per_minute in config ('{custom_max_requests_per_minute}'), using default."  # noqa: E501
        )
        new_max_requests_per_minute = 6
...
def _validate_reports_channel(config_module):
    ...
    if (
        ...
        try:
            ...
            logger.info(
                f"Reports channel ID configured: {
                    config_module.reports_channel_id}"
            )
        except (ValueError, TypeError):
            logger.warning(
                f"Invalid reports_channel_id in config: '{
                    config_module.reports_channel_id}'. It should be an integer. Reports will not be posted."  # noqa: E501
            )
...
def _validate_summary_time(config_module):
    ...
    try:
        ...
        if not (0 <= sh <= 23 and 0 <= sm <= 59):
            logger.warning(
                f"Invalid summary_hour ({sh}) or summary_minute ({sm}) in config. Using default 00:00 UTC."  # noqa: E501
            )
...
def validate_config(config_module):
    ...
    _validate_required_string(
        config_module, "token", min_length=50, display_name="Discord token"
    )

    # Validate LLM configuration
    _validate_required_string(config_module, "llm_api_key", display_name="LLM API key")
    _validate_required_string(
        config_module, "llm_base_url", display_name="LLM Base URL"
    )
    _validate_url(config_module, "llm_base_url", display_name="LLM Base URL")
    _validate_required_string(
        config_module, "llm_model", min_length=1, display_name="LLM Model"
    )

    # Log LLM configuration
    logger.info(
        f"Using LLM model: {
            config_module.llm_model} at {
            config_module.llm_base_url}"
    )

    # Validate Firecrawl API key
    _validate_required_string(
        config_module, "firecrawl_api_key", display_name="Firecrawl API key"
    )

    # Validate optional Apify API token
    if _validate_optional_string(
        config_module, "apify_api_token", display_name="Apify API token"
    ):
        logger.info(
            "Apify API token found in config.py. Twitter/X.com links will be processed using Apify."  # noqa: E501
        )
    else:
        logger.info(
            "Apify API token not found in config.py. Twitter/X.com links will be processed using Firecrawl."  # noqa: E501
        )
