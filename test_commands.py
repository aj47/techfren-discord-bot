...
class TestBotCommands(unittest.TestCase):
    ...
    def test_handle_sum_day_command(self):
        """Test handle_sum_day_command function"""
        # Create mock message
        channel = MagicMock()
        ...
        from command_handler import handle_sum_day_command

        # Process the message
        import asyncio
        asyncio.run(handle_sum_day_command(message, self.client.user))

        # Check if the bot responded
        channel.send.assert_called()
        self.mock_call_llm_for_summary.assert_called_once()

    def test_handle_sum_week_command(self):
        """Test handle_sum_week_command function"""
        # Create mock message
        channel = MagicMock()
        channel.name = "test-channel"
        channel.id = "test_channel_id"
        ...
        # Import the function to test
        from command_handler import handle_sum_week_command

        # Process the message
        import asyncio
        asyncio.run(handle_sum_week_command(message, self.client.user))
